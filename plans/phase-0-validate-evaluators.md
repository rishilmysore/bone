...
1. [ ] `evals/evaluators.py` (stdlib-only: EvalResult dataclass + exit_code, diff_stats, token_count, file_exists) + `evals/conftest.py`. Env: `python3 -m venv .venv && .venv/bin/pip install pytest pyyaml`; record resolved versions here.
2. [ ] `evals/fixtures/` — F1–F5 exactly as the matrix specifies.
3. [ ] `evals/datasets/*.yaml` (4 files, 17 cases, pydantic-evals case shape) + `evals/test_discrimination.py` (YAML-driven runner + `test_acceptance_criterion` meta-test). Suite green.
